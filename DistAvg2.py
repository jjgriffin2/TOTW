...
class Node(object):
    ...
    def processXexhange(self,message):
...
#        breakpoint()
    ...
    def processDdelta(self,message):
        
        messageAck = nodeMessage(5,self.nodeID,message.fromNode,[],0)
        self.sQueue.enqueue(messageAck)
        value = message.value
        nodeFrom = message.fromNode
        for i in range(self.nNeighbors):
...
#        breakpoint()
        ...
        #     x += node.x * densityFactor
        # x += self.x * (1 - sumDensityFactor)
        # self.x = x
    ...
    def sendUpdateX(self):
#        breakpoint()
        for node in self.neighborNodes:
            message = nodeMessage(3, self.nodeID, node.nodeID, [],
                                  self.x,self.density)
            self.sQueue.enqueue(message)
